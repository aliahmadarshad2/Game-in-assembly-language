...
.data
    ...
    msg1 db CR,LF,'===========================================================$' 
    msg2 db CR,LF,'||                 GUESS THE NUMBER                       ||$'  
    msg3 db CR,LF,'||->The digits add up to a number less than 20            ||$' 
    msg4 db CR,LF,'||->The second digit is divisible by 2 and 3              ||$'
    msg5 db CR,LF,'||->The First digit is smaller than second and third digit||$'
    prompt      db  CR, LF,'Please enter a valid number : $'
    lessMsg     db  CR, LF,'Value if Less ','$'
    moreMsg     db  CR, LF,'Value is More ', '$'
    equalMsg    db  CR, LF,'"CONGRATULATION!!!" You have made fine Guess!', '$'
...
function proc
 start:
    ...
    MOV ds, ax   
    MOV dx, offset msg1   ; load address of 'msg1' message to DX
 
    MOV ah, 9h              ; Write string to STDOUT (for DOS interrupt)
    INT 21h             ; set 'data segment' to value of AX which is 'address of data' 
        MOV dx, offset msg1   ; load address of 'msg1' message to DX
 
    MOV ah, 9h              ; Write string to STDOUT (for DOS interrupt)
    INT 21h   
        MOV dx, offset msg2   ; load address of 'msg2' message to DX
 
    MOV ah, 9h              ; Write string to STDOUT (for DOS interrupt)
    INT 21h 
        MOV dx, offset msg3   ; load address of 'msg13' message to DX
 
    MOV ah, 9h              ; Write string to STDOUT (for DOS interrupt)
    INT 21h     
        MOV dx, offset msg4   ; load address of 'msg4' message to DX
 
    MOV ah, 9h              ; Write string to STDOUT (for DOS interrupt)
    INT 21h 
      MOV dx, offset msg5   ; load address of 'msg5' message to DX
    ...
    MOV ah, 9h              ; Write string to STDOUT (for DOS interrupt)
    INT 21h             ; set 'data segment' to value of AX which is 'address of data' 
        MOV dx, offset msg1   ; load address of 'prompt' message to DX
...
while:
    ...
    CMP     cl, 3d          ; compare CL with 3d (3 is the maximum number of digits allowed)
    JG      endwhile        ; IF CL > 3 then JUMP to 'endwhile' label
...
while2:
 ...
 while3:
    ...
    MUL dh                  ; AX = AL * DH which is = to (AL * 10)
